#########################################################################################################################################
# Last Date Modified: 6/22/20
# Author: Jasmine Barrera, Joe Sanchez
# Description: Classification Tree Construction (for SDSU course)

# Spam email Dataset Analysis (from UCI Machine Learning Repository)
# Tasks:
# 1) Contruct a classifier for predicting spam emails.
# 2) Apply a loss matrix to create a second classifier.
...
###########################
# TASK 1) FIRST CLASSIFIER
###########################

## Split data into training and testing samples, using stratified sampling
...
## Grow and prune tree with training data (aka fit classification tree to spam data using 10fold xvalidation)
...
legend("topright", c("trainerror","xerror"), lty=c(1,2)) # coordinates are for top left corner of legend
...
## Run test sample down optimal tree
pred1<-predict(fit1pruned,newdata=test,type="class")
...
xerror2 <- (fit2$cptable[,4])/10
xstd2 <- (fit2$cptable[,5])/10

plot(numsplits2, trainerror2, ylim=c(0.05, 1.2), type="l") # plot training error (solid line)
...
legend("topright", c("trainerror2","xerror2"), lty=c(1,2)) # coordinates are for top left corner of legend
...
# Run test sample down optimal tree
pred2<-predict(fit2pruned,newdata=test,type="class")
...
# The first classifier had a smaller total error (summation of false positives and false negatives), but 
# the second classifier had a smaller number of false positives. Thus, if reducing the number of false positives,
# (emails that were classified as spam when really legitimate) is prioritized over reducing the total error,
# the second classifier with the loss matrix should be used. 
